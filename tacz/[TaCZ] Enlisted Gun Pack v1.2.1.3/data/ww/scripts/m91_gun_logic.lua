...
local function getReloadTimingFromParam(param)
    ...
    local intro = param.intro * 500
    local intro_empty = param.intro_empty * 500
    local intro_empty_clip = param.intro_empty_clip * 500
    local loop = param.loop * 600
    ...
    local loop_feed = param.loop_feed * 400
    -- Check if any timing is nil
    if (intro == nil or intro_empty == nil or intro_empty_clip == nil or loop == nil or ending == nil or ending_empty == nil or ending_empty_feed == nil or clip_load == nil or clip_load_feed == nil or loop_feed == nil) then
        return nil
    end
    return intro, intro_empty, intro_empty_clip, loop, ending, ending_empty, ending_empty_feed, clip_load, clip_load_feed, loop_feed
end

function M.tick_reload(api)
    -- Get all timings from script parameter in gun data
    local param = api:getScriptParams();
    local intro, intro_empty, intro_empty_clip, loop, ending, ending_empty, ending_empty_feed, clip_load, clip_load_feed, loop_feed = getReloadTimingFromParam(param)
    ...
    if (reloaded_count >= 0) then
        ...
        if(reloaded_count >= 5 and api:getAttachment("SCOPE") == "tacz:empty") then
            base_time = (math.floor(reloaded_count / 5) * clip_load) + (reloaded_count % 5) * loop
        end
        if (cache.needed_count - reloaded_count >= 5 and api:getAttachment("SCOPE") == "tacz:empty") then
            base_time = base_time + clip_load_feed
        else
            base_time = base_time + loop_feed
        end
        if (not cache.is_tactical) then
            if (api:getAttachment("SCOPE") == "tacz:empty") then
                base_time = base_time + intro_empty_clip
            else
                if (api:getMaxAmmoCount() > 5) then
                    base_time = base_time + intro_empty_clip
                else
